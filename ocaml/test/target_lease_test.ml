...
let () =
  ...
  assert (
    String.equal
      ...
      (render_client_message (Acquire request)));
  assert (Result.is_error (uuid_of_string (String.uppercase scope)));
  assert (
    Result.is_error (uuid_of_string "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeeg"));
  assert (
    Result.is_error (uuid_of_string "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeeZ"));
  assert (
    Result.is_error
      (uuid_of_string
         ("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeee"
         ^ String.of_char (Char.of_int_exn 0x80))));
  ...
  assert (
    Result.is_error (parse_client_line (String.make (max_line_bytes + 1) 'x')));
  let ready =
    Ready
      {
        authority = assert_ok (uuid_of_string uuid);
        scope = assert_ok (uuid_of_string scope);
        operation = assert_ok (uuid_of_string operation);
        receipt = assert_ok (uuid_of_string receipt);
        identity = assert_ok (uuid_of_string identity);
      }
  in
  ...
  assert (
    Result.is_error
      (parse_response_line "V1 READY 11111111-2222-3333-4444-555555555555"));
