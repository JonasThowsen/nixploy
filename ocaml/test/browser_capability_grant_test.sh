...
# Renew well before the server's fixed five-minute TTL and fail closed after a
# failed renewal or reconnect rather than retaining the prior token.
grep -Fq '~every:(Time_ns.Span.of_sec 30.) capabilities_query' "$app"
grep -Fq 'set_capability_grant ""' "$app"
