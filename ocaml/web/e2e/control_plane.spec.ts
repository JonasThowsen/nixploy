...
test("direct routes and reload retain the selected application", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "All host details" }).click();
  ...
  await expect(page.locator(".application-page .page-intro h2")).toHaveText(key);
  await page.reload();
  await expect(page).toHaveURL(`${baseURL}/apps/${key}`);
  await expect(page.locator("#page-heading")).toHaveText(key);
  await expect(page.locator(".application-page .page-intro h2")).toHaveText(key, { timeout: 60_000 });
});
...
test("SPA navigation, Back, and Forward synchronize URL, navigation, and headings", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Host health", exact: true }).click();
  await expect(page).toHaveURL(`${baseURL}/telemetry`);
  await expect(page.locator("#page-heading")).toHaveText("Host health");
  await expect
    .poll(() => page.locator("#main-content").evaluate((main) => main.scrollTop))
    .toBe(0);
  await expect(page).toHaveTitle("Host health · nixploy");
