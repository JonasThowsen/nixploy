defmodule Nixploy.Operations.StatusWorker do
  ...
  @impl Oban.Worker
  def perform(%Oban.Job{args: %{"service_id" => service_id, "request_id" => request_id}}) do
    service = Applications.get_service!(service_id)

    case StatusProbe.observe(service) do
      {:ok, observed} ->
        case Operations.complete_status_refresh(service.id, request_id, observed) do
          {:ok, _observation} -> :ok
          {:error, reason} -> {:error, inspect(reason)}
        end

      {:error, reason} ->
        _ = Operations.fail_status_refresh(service.id, request_id, reason)
